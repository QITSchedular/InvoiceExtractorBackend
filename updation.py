    ...
    "template_1": {                    
        ...
        "table_structure": {
                "columns": {
                    ...
                    "QTY.": ["QTY", "Quantity", "Item Quantity", "Qry."],  # Added "Qry."
...
# [INFO] Processing template: template_1
# [ERROR] An error occurred: extract_table_data() missing 1 required positional argument: 'table_structure'
# hai toh table_structure
"template_4": {                       
    "keys": {
        "MSME": [
            "MSME NO",
...
def extract_table_data(pdf_path):
    """Extract table data with row-wise coordinates and combine them at the end."""
    table_data = []
    seen_sr_numbers = set()        # Set to keep track of already seen serial numbers
    last_coordinates = {}          # Store the last coordinates found for each column
    used_coordinates = []          # Track used coordinates to prevent overlap

    # Helper function to decide if a row looks like a header extension.
    def is_header_extension(row):
        non_empty = [cell for cell in row if cell.strip() != ""]
        if not non_empty:
            return False
        digit_count = sum(1 for cell in row if any(ch.isdigit() for ch in cell))
        return digit_count < (len(row) * 0.4)
    ...
    try:
        ...
        for table in tables:
            ...
            header_row = None
            header_index = None
            header_variants = ["sr", "sr no", "no"]

            for i, row in enumerate(extracted_table):
                normalized_row = [clean_text(cell) for cell in row]
                if not normalized_row:
                    continue
                first_cell = normalized_row[0].strip().lower() if normalized_row[0] else ""
                if first_cell in header_variants or first_cell.startswith("sr"):
            ...
            if not header_row:
                for i, row in enumerate(extracted_table):
                    normalized_row = [clean_text(cell) for cell in row]
                    if any("sr" in cell.lower() for cell in normalized_row):
                        header_row = normalized_row
                        header_index = i
                        break

            if not header_row:
                # print("[WARNING] No valid headers found in this table. Skipping.")
                continue

            current_index = header_index
            while current_index + 1 < len(extracted_table):
                next_row = [clean_text(cell) for cell in extracted_table[current_index + 1]]
                first_cell_next = next_row[0].strip().lower() if next_row and next_row[0] else ""
                if is_header_extension(next_row) or first_cell_next in ["no", "no."]:
                    min_len = min(len(header_row), len(next_row))
                    merged_header = [f"{header_row[i]} {next_row[i]}".strip() for i in range(min_len)]
                    if len(header_row) > min_len:
                        merged_header.extend(header_row[min_len:])
            ...
            print(f"[INFO] Merged Header: {header_row}")

            valid_headers = set(header_row)

            # New logic to handle coordinates for table data
                ...
                for row_idx in range(max_split_count):
                    ...
                    for idx in range(len(header_row)):
                        ...
                        if field_name.lower().replace(" ", "").replace(".", "") in ["srno", "sr"]:
                            ...
                            # Get coordinates for the header field and its values
                            ...
                            if field_coordinates and field_coordinates["x"] is not None:
                                ...
                                used_coordinates.append(field_coordinates)  # Track used coordinates to prevent overlap
...
def extract_invoice_data_with_table(pdf_path, text, template):
    ...
    if not text:
        # print("[ERROR] The 'text' variable is empty or None!")
        return {}

    # print("[INFO] Text received in extract_invoice_data_with_table:")
    # print(text[:5000]) 
    ...
    if is_text_based(pdf_path):
        table_data, combined_row_coordinates = extract_table_data(pdf_path)  # Unpacking 
    else:
        extracted_text = extract_table_from_image(pdf_path)
        table_data = extracted_text  # Store extracted OCR text as table data
        combined_row_coordinates = None  # No coordinates for image-based extraction

    extracted_data['Table Data'] = table_data  # Store only table data 
    extracted_data['Combined Row Coordinates'] = combined_row_coordinates  # Store combined coordinates 
...
if __name__ == "__main__":        
    pdf_path = r'/Users/quantumitservicesllp/Desktop/invoice_project/invoices/invoice_11.pdf'         
